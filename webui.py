...
#                        ValueError: orginal coding "ValueError" is not recognized
                    ...
                    with gr.Tab(label='Expert Tools'):
                        ...
                        if not args_manager.args.disable_metadata:
                            save_metadata_to_images = gr.Checkbox(label='Save Metadata to Images', value=modules.config.default_save_metadata_to_images,
                                            info='Adds parameters to an image so that it can be regenerated. A Metadata Scheme is not in effect unless this box is checked.')
        ...
        if not args_manager.args.disable_preset_selection and preset_selection != '':
            preset_selection.change(_change, inputs=[preset_selection, state_is_generating, inpaint_mode], outputs=load_data_outputs, queue=False, show_progress=True) \
